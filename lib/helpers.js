    ...
    try {
        ...
        if (publicKeyCbor[3] == -7) {
            publicKeyJwk = {
                kty: "EC",
                crv: "P-256",
                x: base64encode(publicKeyCbor[-2]),
                y: base64encode(publicKeyCbor[-3])
            }
        } else if (publicKeyCbor[3] == -257) {
            publicKeyJwk = {
                kty: "RSA",
                n: base64encode(publicKeyCbor[-1]),
                e: base64encode(publicKeyCbor[-2])
            }
        } else {
 ...
 * @return {any} credential object
	...
	1. Check that challenge is set to the challenge you’ve sent
	2. Check that origin is set to the the origin of your website. If it’s not raise the alarm, and log the event, because someone tried to phish your user
	3. Check that type is set to either “webauthn.create” or “webauthn.get”.
	4. Parse authData or authenticatorData.
	5. Check that flags have UV or UP flags set.

	If the type is set to “webauthn.create”:
	- Verify attestation:
	- Packed: (TODO)
	- FIDO-U2F: https://medium.com/@herrjemand/verifying-fido-u2f-attestations-in-fido2-f83fab80c355
	- Android Keystore: (TODO)
	- Android SafetyNet: (TODO)
	- TPM: https://medium.com/@herrjemand/verifying-fido-tpm2-0-attestation-fc7243847498
